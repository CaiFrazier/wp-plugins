 ...
 * before this consolidation each AJAX entry point re-implemented
...
final class Security {
	...
	public static function authorize_ajax(): void {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( __( 'Unauthorized', 'cf-media-optimizer' ), 403 );
		}
	}
	...
	public static function authorize_ajax_network( string $nonce_action ): void {
		...
		if ( ! current_user_can( $cap ) ) {
			wp_send_json_error(
				is_multisite()
					? __( 'Unauthorized — requires network-admin capability.', 'cf-media-optimizer' )
					: __( 'Unauthorized', 'cf-media-optimizer' ),
				403
			);
		}
	}
