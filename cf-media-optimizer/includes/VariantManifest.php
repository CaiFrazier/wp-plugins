...
class VariantManifest {
	 ...
	 * from cf-webp to cf-media-optimizer, which was a clean break — no
	 * cf-media-optimizer install can have legacy rows.
	 *
		...
		foreach ( $writes as $row ) {
			...
			if ( false === $result ) {
				...
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG && 1 === $failures ) {
					...
					error_log( '[cf-media-optimizer] backfill insert failed for post ' . (int) $row['post_id'] . ': ' . (string) $wpdb->last_error );
				}
			}
		}
		...
		foreach ( $writes as $row ) {
			...
			if ( ! $paths->within_upload_dir( $abs ) ) {
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					...
					error_log( '[cf-media-optimizer] refusing manifest write — path escapes uploads dir: ' . $abs );
				}
				continue;
			}
			$kept[] = $row;
