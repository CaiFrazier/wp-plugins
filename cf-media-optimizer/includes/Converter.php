...
final class Converter {
	...
	public function convert( string $src, ?int $quality = null, bool $force = false, ?int $attachment_id = null ): bool {
		...
		if ( ! in_array( $ext, [ 'jpg', 'jpeg', 'png' ], true ) ) {
			$this->last_skip_reason = __( 'Source is not a supported JPG or PNG.', 'cf-media-optimizer' );
			return false;
		}
		...
		if ( ! $this->paths->within_upload_dir( $src ) ) {
			$this->last_skip_reason = __( 'Source path is outside the uploads directory.', 'cf-media-optimizer' );
			return false;
		}
		if ( ! is_readable( $src ) ) {
			$this->last_skip_reason = __( 'Source file is missing or unreadable.', 'cf-media-optimizer' );
			return false;
		}
		...
		if ( $src_size > $max_bytes ) {
			$this->last_skip_reason = sprintf(
				...
				__( 'Source is %1$.1f MB, larger than the %2$d MB cap (Settings → Max source filesize).', 'cf-media-optimizer' ),
				$src_size / 1024 / 1024,
		...
		if ( ! $this->paths->within_upload_dir( $webp_dest ) ) {
			$this->last_skip_reason = __( 'Derived WebP path resolved outside the uploads directory.', 'cf-media-optimizer' );
			return false;
		}
		...
		if ( null !== $avif_breaker_key && function_exists( 'get_transient' ) && get_transient( $avif_breaker_key ) ) {
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				...
				error_log( '[cf-media-optimizer] AVIF circuit-breaker armed for ' . $src . ' — skipping AVIF (a prior encode crashed the worker).' );
			}
		}
		...
		if ( $webp_blocked ) {
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				...
				error_log( '[cf-media-optimizer] Refusing to overwrite unowned variant: ' . $webp_dest );
			}
			$this->last_skip_reason = __( 'An unowned WebP exists in the destination slot. Open the plugin\'s "Diagnose Attachment" tool (Media → Media Optimizer → Convert tab) and enter this image\'s ID for the exact cause and a one-click fix.', 'cf-media-optimizer' );
			return false;
		}
		if ( $avif_blocked ) {
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- WP_DEBUG-gated security event log.
				error_log( '[cf-media-optimizer] Refusing to overwrite unowned variant: ' . $avif_dest );
			}
			$avif_dest = null;
		}
		...
		if ( ! is_array( $info ) || ! isset( $info[2] ) ) {
			$this->last_skip_reason = __( 'getimagesize() failed — source file is corrupt or not a real image.', 'cf-media-optimizer' );
			return false;
		}
		if ( ! in_array( (int) $info[2], [ IMAGETYPE_JPEG, IMAGETYPE_PNG ], true ) ) {
			$this->last_skip_reason = __( 'Source is not a JPEG or PNG (extension matches but file header does not).', 'cf-media-optimizer' );
			return false;
		}
		if ( ( (int) $info[0] ) * ( (int) $info[1] ) > Options::MAX_PIXELS ) {
			$this->last_skip_reason = sprintf(
				/* translators: 1: width, 2: height, 3: pixel cap in megapixels (e.g., 25). */
				__( 'Source is %1$d×%2$d, exceeding the %3$d-megapixel cap (decompression-bomb defense).', 'cf-media-optimizer' ),
				(int) $info[0],
				(int) $info[1],
		...
		if ( ! function_exists( 'imagewebp' ) ) {
			$this->last_skip_reason = $imagick_failed
				? __( 'Imagick failed and GD has no imagewebp() — no WebP encoder available on this host.', 'cf-media-optimizer' )
				: __( 'No WebP encoder available (Imagick missing the WEBP coder and GD has no imagewebp()).', 'cf-media-optimizer' );
			return false;
		}
		...
		try {
			if ( $ext === 'png' ) {
				...
				if ( ! $img ) {
					$this->last_skip_reason = __( 'GD imagecreatefrompng() failed — source PNG is corrupt or unsupported.', 'cf-media-optimizer' );
					return false;
				}
				...
				if ( ! $img ) {
					$this->last_skip_reason = __( 'GD imagecreatefromjpeg() failed — source JPEG is corrupt or unsupported.', 'cf-media-optimizer' );
					return false;
				}
				$ok = imagewebp( $img, $webp_dest, $quality );
			}
			...
			$this->last_skip_reason = __( 'GD imagewebp() returned false — encoder rejected the output (check disk space and write permissions).', 'cf-media-optimizer' );
			return false;
		} catch ( Throwable $e ) {
			$this->last_skip_reason = sprintf(
				/* translators: %s: PHP exception message from the GD encoder. */
				__( 'GD encoder threw: %s', 'cf-media-optimizer' ),
				$e->getMessage()
			);
			return false;
		}
	...
	public function convert_and_measure( string $src, bool $force, int &$converted, int &$failed, int &$gd_fallbacks, int &$avif_written, ?int $attachment_id = null, ?array &$reasons = null ): int {
		...
		if ( ! $this->paths->within_upload_dir( $src ) ) {
			++$failed;
			$add_reason( __( 'Source path is outside the uploads directory.', 'cf-media-optimizer' ) );
			return 0;
		}
		if ( ! is_readable( $src ) ) {
			++$failed;
		...
		if ( ! $this->paths->within_upload_dir( $webp ) ) {
			++$failed;
			$add_reason( __( 'Derived WebP path resolved outside the uploads directory.', 'cf-media-optimizer' ) );
			return 0;
		}
	...
	public function convert_attachment( int $attachment_id, bool $force = false ): array {
		...
		$blocked    = array(
			...
			0,
			0,
			array( __( 'Conversion already in progress for this attachment (another worker holds the lock).', 'cf-media-optimizer' ) ),
		);
		...
		try {
			...
			if ( ! $file ) {
				$reasons[] = __( 'Attachment has no source file recorded (get_attached_file returned empty).', 'cf-media-optimizer' );
				return array( 0, 0, 0, 0, 0, $reasons );
			}
			if ( ! $meta ) {
				$reasons[] = __( 'Attachment has no metadata (wp_get_attachment_metadata returned empty).', 'cf-media-optimizer' );
				return array( 0, 0, 0, 0, 0, $reasons );
			}
