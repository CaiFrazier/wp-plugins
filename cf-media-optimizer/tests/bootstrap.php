...
// exercised by unit tests. The failure side throws a CFMediaOptimizerHttpExit
// exception in place of wp_die() so the test process survives and assertions
// can inspect the would-have-been HTTP response.
// -----------------------------------------------------------------------------

if ( ! class_exists( 'CFMediaOptimizerHttpExit' ) ) {
	final class CFMediaOptimizerHttpExit extends \RuntimeException {
		public bool $success;
		public $data;
		public int $status;
...
if ( ! function_exists( 'wp_send_json_success' ) ) {
	function wp_send_json_success( $data = null, $status = 200 ): void {
		throw new \CFMediaOptimizerHttpExit( true, $data, (int) $status );
	}
}
...
if ( ! function_exists( 'wp_send_json_error' ) ) {
	function wp_send_json_error( $data = null, $status = 200 ): void {
		throw new \CFMediaOptimizerHttpExit( false, $data, (int) $status );
	}
}
...
if ( ! function_exists( 'check_ajax_referer' ) ) {
	function check_ajax_referer( $action = -1, $query_arg = false, $die = true ) {
		...
		if ( ! empty( $GLOBALS['cf_media_manager_test_nonce_invalid'] ) ) {
			if ( $die ) {
				throw new \CFMediaOptimizerHttpExit( false, 'bad nonce', 403 );
			}
			return false;
		}
		return 1;
...
if ( file_exists( $autoload ) ) {
	...
	spl_autoload_register(
		static function ( $class ) {
			$map = array(
				'CFMediaOptimizer\\' => __DIR__ . '/../includes/',
