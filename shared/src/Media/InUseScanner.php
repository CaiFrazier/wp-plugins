...
final class InUseScanner {
	...
	// Scanner-owned option keys. Neutral (cf_media_*, not per-plugin) so that
	// when both CF Media Optimizer and CF Media Manager are installed they
	// share one warm cache + builder fingerprint instead of double-scanning.
	const OPT_WARM_WANTED     = 'cf_media_in_use_warm_wanted';
	const OPT_LAST_BUILDER_FP = 'cf_media_last_builder_fp';
	...
	private function maybe_schedule_warm(): void {
		...
		$wanted = (int) get_option( self::OPT_WARM_WANTED, 0 );
	...
	public function get_for_report( bool $force_refresh = false ): array {
		if ( function_exists( 'update_option' ) ) {
			update_option( self::OPT_WARM_WANTED, time(), false );
		}
	...
	public function invalidate_all_fingerprinted_caches(): void {
		...
		$last_fp    = function_exists( 'get_option' ) ? (string) get_option( self::OPT_LAST_BUILDER_FP, '' ) : '';
		...
		if ( ! isset( $wpdb ) || ! is_object( $wpdb ) ) {
			...
			if ( function_exists( 'update_option' ) ) {
				update_option( self::OPT_LAST_BUILDER_FP, $current_fp, true );
			}
			return;
		}
