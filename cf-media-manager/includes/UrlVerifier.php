...
final class UrlVerifier {
	...
	public static function fetch( string $url ): array {
		...
		for ( $hop = 0; $hop <= Options::VERIFY_MAX_HOPS; $hop++ ) {
			...
			if ( empty( $resolved ) ) {
				return self::error( __( 'Target host did not resolve.', 'cf-media-manager' ), 502 );
			}
			foreach ( $resolved as $ip ) {
				if ( ! self::ip_is_publicly_routable( $ip ) ) {
					return self::error( __( 'Target resolves to a private or loopback address; refusing to fetch.', 'cf-media-manager' ), 502 );
				}
			}
			...
			if ( is_wp_error( $response ) ) {
				return self::error(
					sprintf(
						...
						__( 'Fetch failed: %s', 'cf-media-manager' ),
			...
			if ( Options::VERIFY_MAX_HOPS === $hop ) {
				return self::error( __( 'Too many redirects.', 'cf-media-manager' ), 502 );
			}
		...
		if ( $code < 200 || $code >= 400 ) {
			return self::error(
				sprintf(
					...
					__( 'HTTP %d from target URL.', 'cf-media-manager' ),
					$code
				),
				502
			);
		...
		if ( '' === $body ) {
			return self::error( __( 'Empty response body.', 'cf-media-manager' ), 502 );
		}
	...
	private static function validate( string $url ): array {
		...
		if ( ! is_array( $parts ) || empty( $parts['scheme'] ) || empty( $parts['host'] ) ) {
			return self::error( __( 'Invalid URL.', 'cf-media-manager' ), 400 );
		}

		$scheme = strtolower( (string) $parts['scheme'] );
		if ( 'http' !== $scheme && 'https' !== $scheme ) {
			return self::error( __( 'Only http and https are accepted.', 'cf-media-manager' ), 400 );
		}

		$site_host = strtolower( (string) wp_parse_url( home_url(), PHP_URL_HOST ) );
		$req_host  = strtolower( (string) $parts['host'] );
		if ( '' === $site_host ) {
			return self::error( __( 'Site host could not be determined.', 'cf-media-manager' ), 500 );
		}
		if ( $req_host !== $site_host ) {
			return self::error( __( 'Redirect to off-site host blocked.', 'cf-media-manager' ), 502 );
		}
		...
		if ( preg_match( self::PATH_DENYLIST_REGEX, $path ) ) {
			return self::error( __( 'Admin, login, XML-RPC, and REST paths are not allowed.', 'cf-media-manager' ), 403 );
		}
		...
		if ( '' !== $query ) {
			...
			if ( isset( $qparams['rest_route'] ) ) {
				return self::error( __( 'REST API paths are not allowed.', 'cf-media-manager' ), 403 );
			}
			if ( isset( $qparams['_wpnonce'] ) ) {
				return self::error( __( 'URLs containing _wpnonce are not allowed.', 'cf-media-manager' ), 403 );
			}
		}
	...
	private static function do_http( string $url, string $pinned_ip, string $host ) {
		$args = array(
			...
			'user-agent'          => 'CF-WebP-Verifier/' . ( defined( 'CF_MEDIA_MANAGER_VERSION' ) ? CF_MEDIA_MANAGER_VERSION : 'dev' ),
			'redirection'         => 0,
