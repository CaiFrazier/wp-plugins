...
final class Ajax {
	...
	public function backfill_manifest(): void {
		...
		if ( ! $dry_run ) {
			if ( '' === $cursor && false !== get_transient( Options::BACKFILL_LOCK ) ) {
				wp_send_json_error(
					__( 'A backfill run is already in progress (or completed within the last 10 minutes). Wait for it to finish, or try again after the lock expires.', 'cf-media-manager' ),
					409
				);
			}
		...
		foreach ( $ids as $id ) {
			...
			if ( ! $file ) {
				$statuses[ $id ] = array(
					...
					'label'     => sprintf( __( 'ID %d', 'cf-media-manager' ), (int) $id ),
					'converted' => false,
				);
				continue;
			}
	...
	public function verify_url(): void {
		...
		if ( ! $raw ) {
			wp_send_json_error( __( 'Missing or invalid URL.', 'cf-media-manager' ), 400 );
		}
		...
		if ( ! is_array( $parts ) ) {
			wp_send_json_error( __( 'Invalid URL.', 'cf-media-manager' ), 400 );
		}
		$path  = $parts['path'] ?? '/';
		...
		if ( empty( $result['ok'] ) ) {
			wp_send_json_error( $result['error'] ?? __( 'Fetch failed.', 'cf-media-manager' ), (int) ( $result['status'] ?? 502 ) );
		}
		...
		if ( ! in_array( $mime, array( 'image/jpeg', 'image/png' ), true ) ) {
			wp_send_json_error(
				sprintf(
					...
					__( 'Attachment %s is not a JPEG or PNG.', 'cf-media-manager' ),
					'' === $mime ? 'unknown' : $mime
				),
				400
			);
	...
	public function claim_variant(): void {
		...
		if ( '' === $source_abs || ! file_exists( $source_abs ) ) {
			wp_send_json_error( __( 'Source file not found on disk.', 'cf-media-manager' ), 404 );
		}
	...
	private function summarize_diagnosis( array $report ): string {
		...
		if ( ! ( $source['exists'] ?? false ) ) {
			return __( 'Source file is missing on disk.', 'cf-media-manager' );
		}
		if ( 0 === $source_id && 0 === $source_size ) {
			return __( 'Source rel path is not in either lookup map — the path in _wp_attached_file likely differs from the on-disk filename (case, encoding, or normalization).', 'cf-media-manager' );
		}
		if ( $source_id > 0 && $source_id !== $id ) {
			return sprintf(
				/* translators: 1: this attachment id, 2: id the lookup returned. */
				__( 'Source rel path maps to attachment %2$d, not %1$d. Two attachments may share the same _wp_attached_file value.', 'cf-media-manager' ),
				$id,
				$source_id
			);
		}
		if ( ! ( $webp['exists'] ?? false ) ) {
			return __( 'No WebP at destination slot — nothing to adopt; Convert would proceed normally.', 'cf-media-manager' );
		}
		...
		if ( $webp['owned_by_this_id'] ?? false ) {
			return __( 'WebP exists and is owned by this attachment — Convert should not be blocked. Try Convert again.', 'cf-media-manager' );
		}
		return __( 'WebP exists, source resolves correctly, but the manifest row is missing. Click "Claim this WebP" to fix.', 'cf-media-manager' );
	}
}
