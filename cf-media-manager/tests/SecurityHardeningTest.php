...
use CFMediaManager\Ajax;
use CFMediaManager\CachePurger;
...
use CFMediaManager\Security;
use CFMediaManager\UrlVerifier;
 ...
 *     converts into a CFMediaManagerHttpExit exception so the test process
...
final class SecurityHardeningTest extends TestCase {
	...
	public function test_network_gate_blocks_when_no_caps_on_single_site(): void {
		...
		try {
			Security::authorize_ajax_network( Plugin::NONCE_ACTION );
			self::fail( 'Expected CFMediaManagerHttpExit was not thrown' );
	...
	public function test_network_gate_blocks_manage_options_only_on_multisite(): void {
		...
		try {
			Security::authorize_ajax_network( Plugin::NONCE_ACTION );
			self::fail( 'Expected CFMediaManagerHttpExit — manage_options must NOT be enough on multisite' );
		} catch ( CFMediaManagerHttpExit $e ) {
			self::assertFalse( $e->success );
			self::assertSame( 403, $e->status );
	...
	public function test_network_gate_rejects_invalid_nonce(): void {
		...
		try {
			Security::authorize_ajax_network( Plugin::NONCE_ACTION );
			self::fail( 'Expected CFMediaManagerHttpExit on bad nonce' );
		} catch ( CFMediaManagerHttpExit $e ) {
			self::assertFalse( $e->success );
			self::assertSame( 403, $e->status );
		}
	...
	public function test_network_gate_blocks_full_single_site_admin_on_multisite(): void {
		...
		try {
			Security::authorize_ajax_network( Plugin::NONCE_ACTION );
			self::fail( 'Expected CFMediaManagerHttpExit' );
		} catch ( CFMediaManagerHttpExit $e ) {
			self::assertSame( 403, $e->status );
		}
	}
	...
	public function test_backfill_overlap_lock_rejects_concurrent_fresh_start(): void {
		...
		try {
			...
			self::fail( 'Expected CFMediaManagerHttpExit for concurrent backfill click' );
		} catch ( CFMediaManagerHttpExit $e ) {
			self::assertFalse( $e->success );
