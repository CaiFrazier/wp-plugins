 ...
 * into `cf_media_manager_run_uninstall()` so this test can drive it
 * without re-executing the file. The test does:
 *
 *   1. Seed `is_multisite` to true, `get_sites` to return N blog IDs.
 *   2. Call `cf_media_manager_run_uninstall()` with the canonical
 *      option list.
...
final class UninstallTest extends TestCase {
	...
	protected function setUp(): void {
		...
		// The require above ran cf_media_manager_run_uninstall once with
	...
	public function test_single_site_runs_cleanup_once_and_skips_site_options(): void {
		...
		cf_media_manager_run_uninstall( $this->options );
	...
	public function test_per_site_cleanup_removes_every_option_in_list(): void {
		...
		cf_media_manager_uninstall_cleanup( array( 'cf_media_manager_quality', 'cf_media_manager_queue_state' ) );
